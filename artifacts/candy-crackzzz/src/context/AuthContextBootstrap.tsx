...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const loadAuth = async () => {
      try {
        ...
        if (!isMounted) return;
        applyAuthSnapshot({ isAdminSetup: false, currentUser: null, staffUsers: [], activityLogs: [] });
      } finally {
  ...
  return (
    <AuthContext.Provider value={{ isLoaded, isAdminSetup, isLoggedIn, loginError, currentUser, isOwner, staffUsers, activityLogs, setupAdmin, login, logout, changeCredentials, createEmployeeInvite, setEmployeeAccess, hasRole }}>
      {children}
